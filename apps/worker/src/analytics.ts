...
export class AnalyticsManager {
  ...
  private detectFramework(files: any[]): DetectedFramework {
    const filePaths = files.map(f => f.p || f.name || '').join(' ').toLowerCase();
    
    if (filePaths.includes('dist') && filePaths.includes('index.html')) return 'vite';
    if (filePaths.includes('out') && filePaths.includes('index.html')) return 'nextjs';
    if (filePaths.includes('.svelte-kit')) return 'sveltekit';
    if (filePaths.includes('build') && filePaths.includes('index.html')) return 'create-react-app';
    if (filePaths.includes('vue') || filePaths.includes('.vue')) return 'vue';
    if (filePaths.includes('angular') || filePaths.includes('.ng')) return 'angular';
    if (filePaths.includes('index.html') && files.length <= 5) return 'static';
  ...
  private async incrementUserMetric(userId: string, metric: keyof typeof DEFAULT_USER_ANALYTICS): Promise<void> {
    try {
  ...
  private async incrementSnapshotMetric(snapshotId: string, metric: keyof typeof DEFAULT_SNAPSHOT_ANALYTICS): Promise<void> {
    try {
      const snapshotRaw = await this.env.KV_SNAPS.get(`snap:${snapshotId}`);
      if (!snapshotRaw) return;

      const snapshot: SnapshotRecord = JSON.parse(snapshotRaw);
      ...
      if (typeof snapshot.analytics[metric] === 'number') {
        (snapshot.analytics as any)[metric]++;
      }
  ...
  private async updateSnapshotLastViewed(snapshotId: string): Promise<void> {
    try {
      const snapshotRaw = await this.env.KV_SNAPS.get(`snap:${snapshotId}`);
      if (!snapshotRaw) return;
  ...
  // End view session
  async endViewSession(snapshotId: string, sessionId: string): Promise<void> {
    try {
      const snapshotRaw = await this.env.KV_SNAPS.get(`snap:${snapshotId}`);
      if (!snapshotRaw) return;
  ...
  async getSnapshotAnalytics(snapshotId: string, period: 'day' | 'week' | 'month' | 'year' = 'month'): Promise<SnapshotAnalytics> {
    try {
      const now = Date.now();
      const startDate = this.getPeriodStartDate(now, period);
  ...
  async getSystemAnalytics(period: 'day' | 'week' | 'month' | 'year' = 'month'): Promise<SystemAnalytics> {
    try {
      const now = Date.now();
      const startDate = this.getPeriodStartDate(now, period);
  ...
  private getPeriodStartDate(now: number, period: 'day' | 'week' | 'month' | 'year'): number {
  ...
  private async getEventsForPeriod(userId: string, startDate: number, endDate: number): Promise<AnalyticsEvent[]> {
  ...
  private calculateBounceRate(viewSessions: ViewSession[]): number {
    if (viewSessions.length === 0) return 0;
  ...
  async migrateUserToNewSchema(userId: string): Promise<void> {
    try {
      const userRaw = await this.env.KV_USERS.get(`user:${userId}`);
      if (!userRaw) return;
      ...
      if (user.analytics && user.subscription) return;
  ...
  async migrateSnapshotToNewSchema(snapshotId: string): Promise<void> {
    try {
      const snapshotRaw = await this.env.KV_SNAPS.get(`snap:${snapshotId}`);
      if (!snapshotRaw) return;
      ...
      // Check if already migrated
      if (snapshot.analytics && snapshot.metadata) return;
      ...
      // Add missing timestamp fields
      if (!snapshot.updatedAt) snapshot.updatedAt = snapshot.createdAt;
      if (!snapshot.lastAccessedAt) snapshot.lastAccessedAt = snapshot.createdAt;
