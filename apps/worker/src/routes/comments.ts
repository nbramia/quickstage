...
export async function handleGetSnapshotComments(c: any) {
...
export async function handlePostSnapshotComment(c: any) {
  const id = c.req.param('id');
  if (!id) return c.json({ error: 'bad_request' }, 400);
...
export async function handleGetCommentsLegacy(c: any) {
...
export async function handlePostCommentLegacy(c: any) {
  const body: any = await c.req.json();
...
export async function handleGetSnapshotCommentsAlt(c: any) {
  try {
    const snapshotId = c.req.param('snapshotId');
...
export async function handlePostSnapshotCommentAlt(c: any) {
  try {
    const snapshotId = c.req.param('snapshotId');
