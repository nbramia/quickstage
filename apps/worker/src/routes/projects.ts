...
export async function handleCreateProject(c: any) {
  try {
...
export async function handleGetProjects(c: any) {
  try {
    const uid = await getUidFromSession(c);
    if (!uid) {
      return c.json({ error: 'Unauthorized' }, 401);
...
// Update a project
export async function handleUpdateProject(c: any) {
  try {
    const uid = await getUidFromSession(c);
    if (!uid) {
      return c.json({ error: 'Unauthorized' }, 401);
    ...
    const existingProject = JSON.parse(existingData) as Project;
    ...
    await analytics.trackEvent(
      uid,
      'project_updated',
      { projectId, updates: Object.keys(body) }
    );
...
// Delete a project
export async function handleDeleteProject(c: any) {
  try {
    const uid = await getUidFromSession(c);
    if (!uid) {
      return c.json({ error: 'Unauthorized' }, 401);
    ...
    await analytics.trackEvent(
      uid,
      'project_deleted',
      { projectId, name: existingProject.name }
    );
...
export async function handleArchiveProject(c: any) {
  try {
    const uid = await getUidFromSession(c);
    if (!uid) {
      return c.json({ error: 'Unauthorized' }, 401);
    ...
    await analytics.trackEvent(
      uid,
      isArchived ? 'project_archived' : 'project_unarchived',
      { projectId }
    );
...
// Reorder projects
export async function handleReorderProjects(c: any) {
  try {
    const uid = await getUidFromSession(c);
    if (!uid) {
      return c.json({ error: 'Unauthorized' }, 401);
