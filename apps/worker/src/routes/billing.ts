...
export async function handleStartTrial(c: any) {
  try {
...
export async function handleBillingStatus(c: any) {
  const uid = await getUidFromSession(c);
...
export async function handleBillingCancel(c: any) {
  const uid = await getUidFromSession(c);
  if (!uid) {
    // Track analytics event for unauthorized access attempt
    try {
...
export async function handleChangePayment(c: any) {
  const uid = await getUidFromSession(c);
  if (!uid) return c.json({ error: 'unauthorized' }, 401);
...
export async function handleBillingPortal(c: any) {
  const uid = await getUidFromSession(c);
  if (!uid) {
    // Track analytics event for unauthorized access attempt
    try {
