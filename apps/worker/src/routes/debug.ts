...
function extractTimestampFromKey(eventKey: string): number | null {
  try {
...
async function handleSmartRecentEventsRetrieval(c: any, requestedLimit: number, startTime?: number) {
        ...
        if (key.name.startsWith('event:evt_')) {
          ...
          if (originalTimestamp) {
            ...
            if (eventRaw) {
              ...
              if (allEvents.length >= requestedLimit) break;
            }
          }
        }
      }
...
async function searchEventsByTimestampRange(c: any, startTime: number, endTime: number) {
  const events = [];
  ...
  for (const chunk of timeChunks.slice(0, 10)) { // Limit to 10 chunks to avoid timeout
    ...
    if (events.length > 1000) break; // Prevent memory issues
  }
...
async function searchEventChunk(c: any, startTime: number, endTime: number) {
  const events = [];
