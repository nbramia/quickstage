...
export async function handleApiSnapshotExtend(c: any) {
...
export async function handleApiSnapshotExpire(c: any) {
  const uid = await getUidFromSession(c);
  if (!uid) return c.json({ error: 'unauthorized' }, 401);
  const id = c.req.param('id');
  const metaRaw = await c.env.KV_SNAPS.get(`snap:${id}`);
...
export async function handleApiSnapshotRotatePassword(c: any) {
  const uid = await getUidFromSession(c);
  if (!uid) return c.json({ error: 'unauthorized' }, 401);
  const id = c.req.param('id');
  const metaRaw = await c.env.KV_SNAPS.get(`snap:${id}`);
...
export async function handleApiGoogleAuth(c: any) {
...
export async function handleApiMe(c: any) {
  const uid = await getUidFromSession(c);
  if (!uid) return c.json({ error: 'unauthorized' }, 401);
...
export async function handleApiSnapshotCreate(c: any) {
