...
export class AnalyticsEventMigration {
   ...
   */
  private isNewFormat(eventKey: string): boolean {
   ...
   */
  private generateNewEventId(originalTimestamp: number, randomId: string): string {
   ...
   */
  private parseOldEventKey(eventKey: string): { timestamp: number; randomId: string } | null {
    try {
      const keyParts = eventKey.replace('event:', '').split('_');
   ...
   */
  private async migrateEvent(eventKey: string, eventData: AnalyticsEvent): Promise<boolean> {
    try {
      ...
      const updatedEventData: AnalyticsEvent = {
        ...eventData,
        id: newEventId
      };
   ...
   */
  private async processBatch(keys: any[]): Promise<void> {
    ...
    for (const key of keys) {
      ...
      try {
        ...
        const eventData: AnalyticsEvent = JSON.parse(eventRaw);
   ...
   */
  async executeMigration(maxBatches?: number): Promise<MigrationStats> {
   ...
   */
  async testMigration(): Promise<MigrationStats> {
