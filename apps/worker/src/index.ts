...
// TODO: Implement migration debug endpoints
...
app.post('/admin/migrate-stripe-production', async (c: any) => {
  if (!(await isSuperadmin(c))) {
    return c.json({ error: 'Superadmin access required' }, 403);
  }
...
app.get('/admin/test-migration-preview', async (c: any) => {
  try {
    ...
    // Get all user keys
    while (hasMore) {
      const listResult: any = await c.env.KV_USERS.list({ 
        prefix: 'user:',
        cursor: cursor
      });
...
app.post('/webhook/stripe', async (c: any) => {
  try {
