export class CommentsRoom {
    ...
    async handleEnhancedComments(request, url, method) {
        const path = url.pathname;
        try {
    ...
    async createEnhancedComment(request) {
    ...
    async listEnhancedComments(request, url) {
    ...
    async getEnhancedComment(request, path) {
    ...
    async updateEnhancedComment(request, path) {
        const commentId = path.split('/').pop();
        if (!commentId) {
            return new Response(JSON.stringify({ error: 'Invalid comment ID' }), {
                status: 400,
    ...
    async deleteEnhancedComment(request, path) {
        const commentId = path.split('/').pop();
        if (!commentId) {
            return new Response(JSON.stringify({ error: 'Invalid comment ID' }), {
                status: 400,
    ...
    async bulkResolveComments(request) {
