...
function injectViewerOverlay(html, snapshotId, isOwner = false) {
    const overlayCSS = `
...
function getPasswordGateHTML(id) {
    return `
<!DOCTYPE html>
<html lang="en">
<head>
...
export async function handleSnapshotGate(c) {
    const id = c.req.param('id');
...
export async function handleSnapshotFile(c, id, fileName) {
    ...
    else if (actualFilePath.endsWith('.js'))
        contentType = 'application/javascript';
    else if (actualFilePath.endsWith('.css'))
        contentType = 'text/css';
    else if (actualFilePath.endsWith('.json'))
        contentType = 'application/json';
    else if (actualFilePath.endsWith('.png'))
        contentType = 'image/png';
    else if (actualFilePath.endsWith('.jpg') || actualFilePath.endsWith('.jpeg'))
        contentType = 'image/jpeg';
    else if (actualFilePath.endsWith('.svg'))
        contentType = 'image/svg+xml';
    else if (actualFilePath.endsWith('.ico'))
    ...
    if (actualFilePath.endsWith('.html') || actualFilePath.endsWith('.css') || actualFilePath.endsWith('.js')) {
        ...
        content = content
            ...
            .replace(/import\s+.*from\s+["']\/([^"']+)["']/g, (match, path) => match.replace(`"/${path}"`, `"/s/${snapshotId}/${path}"`))
            .replace(/import\s+.*from\s+['"]\/([^"']+)["']/g, (match, path) => match.replace(`'/${path}'`, `'/s/${snapshotId}/${path}'`))
