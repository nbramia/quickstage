...
export async function handleSubscribeToSnapshot(c) {
    try {
        ...
        await c.env.KV_SUBSCRIPTIONS.put(`sub:${uid}:${snapshotId}:${commentId || 'snapshot'}`, JSON.stringify(subscription));
...
export async function handleUnsubscribeFromSnapshot(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ error: 'Unauthorized' }, 401);
...
export async function handleGetUserSubscriptions(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ error: 'Unauthorized' }, 401);
...
export async function handleUpdateSubscription(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ error: 'Unauthorized' }, 401);
...
export async function handleDeleteSubscription(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ error: 'Unauthorized' }, 401);
            ...
            if (subRaw) {
                ...
                if (sub.id === subscriptionId) {
                    ...
                    if (userSubsRaw) {
                        ...
                        const filteredSubs = userSubs.filter((id) => id !== subscriptionId);
...
export async function createNotificationForSubscribers(env, snapshotId, commentId, type, title, message, actionUrl) {
    try {
        ...
        for (const key of keys.keys) {
            ...
            if (subRaw) {
                ...
                if (matchesSnapshot && matchesComment) {
                    ...
                    // Store notification
                    await env.KV_NOTIFICATIONS.put(`ntf:${sub.userId}:${notificationId}`, JSON.stringify(notification));
