...
export async function handleCreateReview(c) {
    try {
...
// Get review details
export async function handleGetReview(c) {
    try {
...
export async function handleSubmitReview(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ error: 'Unauthorized' }, 401);
        ...
        const allReviewed = review.reviewers.every(r => r.status === 'approved' || r.status === 'changes_requested');
        const hasChangesRequested = review.reviewers.some(r => r.status === 'changes_requested');
        if (allReviewed) {
        ...
        // Track analytics
        const analytics = getAnalyticsManager(c);
        await analytics.trackEvent(uid, 'review_submitted', {
            reviewId,
            status,
            hasChangesRequested
        });
...
export async function handleGetSnapshotReviews(c) {
    try {
        const snapshotId = c.req.param('snapshotId');
        // Get snapshot
        const snapshotData = await c.env.KV_SNAPS.get(`snap:${snapshotId}`);
...
export async function handleCancelReview(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ error: 'Unauthorized' }, 401);
        ...
        // Track analytics
        const analytics = getAnalyticsManager(c);
        await analytics.trackEvent(uid, 'review_cancelled', { reviewId, snapshotId: review.snapshotId });
...
export async function handleSendReviewReminders(c) {
    try {
        const now = Date.now();
        ...
        for (const key of reviewsList.keys) {
            ...
            if (!review.deadline || review.reminderSent)
                continue;
            ...
            if (timeUntilDeadline > 0 && timeUntilDeadline <= reminderThreshold) {
                ...
                const pendingReviewers = review.reviewers.filter(r => r.status === 'pending' || r.status === 'reviewing');
                if (pendingReviewers.length > 0) {
                    ...
                    // Track analytics
                    const analytics = getAnalyticsManager(c);
                    await analytics.trackEvent('system', 'review_reminder_sent', {
                        reviewId: review.id,
