...
export async function handleCreateSnapshot(c) {
...
export async function handleFinalizeSnapshot(c) {
    let uid = await getUidFromSession(c);
    if (!uid) {
        // Try PAT authentication as fallback
        const authHeader = c.req.header('authorization') || c.req.header('Authorization');
...
export async function handleListSnapshots(c) {
...
export async function handleGetSnapshot(c) {
...
export async function handleExpireSnapshot(c) {
    const uid = await getUidFromSession(c);
    if (!uid)
        return c.json({ error: 'unauthorized' }, 401);
    const id = c.req.param('id');
...
export async function handleExtendSnapshot(c) {
    const uid = await getUidFromSession(c);
    if (!uid)
        return c.json({ error: 'unauthorized' }, 401);
    const id = c.req.param('id');
...
export async function handleRotateSnapshotPassword(c) {
    const uid = await getUidFromSession(c);
    if (!uid)
        return c.json({ error: 'unauthorized' }, 401);
    const id = c.req.param('id');
