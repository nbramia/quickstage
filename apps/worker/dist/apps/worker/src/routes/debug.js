...
export async function handleDebugAnalyticsEvents(c) {
    ...
    try {
        ...
        console.log('🔍 First 5 keys:', list.keys.slice(0, 5).map((k) => k.name));
        const events = [];
...
export async function handleMigrationStatus(c) {
    if (!(await isSuperadmin(c))) {
        return c.json({ error: 'Superadmin access required' }, 403);
    }
...
export async function handleMigrateAnalyticsEventsTest(c) {
    if (!(await isSuperadmin(c))) {
        return c.json({ error: 'Superadmin access required' }, 403);
    }
    try {
...
export async function handleMigrateAnalyticsEventsFull(c) {
    if (!(await isSuperadmin(c))) {
        return c.json({ error: 'Superadmin access required' }, 403);
    }
    try {
