...
export async function handleGetNotifications(c) {
    try {
...
export async function handleMarkNotificationRead(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ error: 'Unauthorized' }, 401);
...
export async function handleMarkAllNotificationsRead(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ error: 'Unauthorized' }, 401);
...
// Delete notification
export async function handleDeleteNotification(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ error: 'Unauthorized' }, 401);
        ...
        if (userNtfsRaw) {
            ...
            const filteredNtfs = userNtfs.filter((id) => id !== notificationId);
...
export async function handleGetNotificationStats(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ error: 'Unauthorized' }, 401);
