...
export async function handleApiExtensionVersion(c) {
    try {
...
export async function handleApiExtensionDownload(c) {
...
export async function handleExtensionVersion(c) {
    try {
        const versionInfo = getExtensionVersion();
        return c.json({
            version: versionInfo.version,
...
export async function handleExtensionDownload(c) {
    // Check authentication first
    const uid = await getUidFromSession(c);
    if (!uid) {
        return c.json({ error: 'unauthorized', message: 'Please log in to download the extension' }, 401);
