...
export async function handleStartAIConversation(c) {
    try {
...
export async function handleSendAIMessage(c) {
    try {
        const snapshotId = c.req.param('snapshotId');
        if (!snapshotId) {
            return c.json({ success: false, error: 'Snapshot ID required' }, 400);
...
export async function handleGetAIConversation(c) {
    try {
        const snapshotId = c.req.param('snapshotId');
        if (!snapshotId) {
            return c.json({ success: false, error: 'Snapshot ID required' }, 400);
...
async function updateRateLimit(c, uid, ipAddress, tokensUsed = 500) {
    const identifier = uid || ipAddress || 'anonymous';
    const hourKey = `rate-limit:${identifier}:${Math.floor(Date.now() / 3600000)}`;
    const current = await c.env.KV_ANALYTICS.get(hourKey, { type: 'json' }) || { requests: 0, tokens: 0 };
    current.requests += 1;
