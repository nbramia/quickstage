...
export async function handleUpdateOnboarding(c) {
    try {
        ...
        if (completedTutorials) {
            ...
            const newTutorials = completedTutorials.filter((t) => !existingTutorials.includes(t));
...
export async function handleGetOnboarding(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ success: false, error: 'Authentication required' }, 401);
...
export async function handleTrackTutorial(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ success: false, error: 'Authentication required' }, 401);
...
export async function handleShouldShowWelcome(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ success: false, error: 'Authentication required' }, 401);
