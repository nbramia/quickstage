    ...
    try {
        ...
        if (contentType.includes('multipart/form-data')) {
            ...
            body = {
                ...
                elementCoordinates: formData.get('elementCoordinates') ? JSON.parse(formData.get('elementCoordinates')) : undefined,
            ...
            attachments = files.filter((file) => file instanceof File);
        }
        else {
...
// Delete comment
export async function handleDeleteComment(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ error: 'Unauthorized' }, 401);
        ...
        if (existingComment.attachments && existingComment.attachments.length > 0) {
            try {
                ...
                do {
                    const objs = await c.env.R2_SNAPSHOTS.list({
                        prefix: `attachments/${snapshotId}/${commentId}/`,
                        cursor: r2cursor
                    });
                    r2cursor = objs.cursor;
                    if (objs.objects.length) {
                        await c.env.R2_SNAPSHOTS.delete(objs.objects.map((o) => o.key));
                    }
                } while (r2cursor);
            }
            catch (error) {
...
// Resolve comment
export async function handleResolveComment(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ error: 'Unauthorized' }, 401);
        ...
        await createNotificationForSubscribers(c.env, snapshotId, commentId, 'comment_resolved', 'Comment resolved', 'A comment thread has been resolved', `${c.env.PUBLIC_BASE_URL}/view/${snapshotId}`);
        return c.json({ success: true, comment: updatedComment });
    }
    catch (error) {
...
export async function handleUploadAttachment(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ error: 'Unauthorized' }, 401);
...
export async function handleBulkResolveComments(c) {
    try {
        const uid = await getUidFromSession(c);
        if (!uid) {
            return c.json({ error: 'Unauthorized' }, 401);
