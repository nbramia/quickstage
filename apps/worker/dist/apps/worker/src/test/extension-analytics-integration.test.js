...
describe('Extension Analytics Integration', () => {
    ...
    it('should update user totalDownloads metric', async () => {
        ...
        const userCall = userUpdateCalls.find((call) => call[0] === 'user:test_user_123');
    ...
    it('should track extension downloads with proper session and user agent', async () => {
        ...
        await analytics.trackEvent('test_user_123', 'extension_downloaded', { version: '1.0.0' }, 'sess_123456789', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36');
        expect(mockEnv.KV_ANALYTICS.put).toHaveBeenCalled();
        const putCalls = mockEnv.KV_ANALYTICS.put.mock.calls;
        const eventCall = putCalls.find((call) => call[0].startsWith('event:'));
        const storedEvent = JSON.parse(eventCall[1]);
