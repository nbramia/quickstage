...
import { handleRegister, handleLogin, handleMe, handleLogout, handleProfile, handleChangePassword } from '../../routes/auth';
...
describe('Auth Routes', () => {
    ...
    describe('handleRegister', () => {
        it('should register a new user successfully', async () => {
            ...
            getUserByName.mockResolvedValue(null); // User doesn't exist
        ...
        it('should return error for missing required fields', async () => {
            ...
            await handleRegister(mockContext);
            expect(mockContext.json).toHaveBeenCalledWith({ error: 'Missing required fields' }, 400);
        });
        it('should return error for existing username', async () => {
            ...
            getUserByName.mockResolvedValue(mockUsers.freeUser); // User exists
            await handleRegister(mockContext);
            expect(mockContext.json).toHaveBeenCalledWith({ error: 'Username already taken' }, 400);
        });
        it('should return error for existing email', async () => {
            ...
            getUserByName.mockResolvedValue(null); // Username available
            mockContext.env.KV_USERS.get.mockResolvedValue('existing_user_id'); // Email exists
            await handleRegister(mockContext);
            expect(mockContext.json).toHaveBeenCalledWith({ error: 'Email already registered' }, 400);
        });
        it('should handle registration errors gracefully', async () => {
            ...
            getUserByName.mockRejectedValue(new Error('Database error'));
            await handleRegister(mockContext);
            expect(mockContext.json).toHaveBeenCalledWith({ error: 'Registration failed' }, 500);
        });
    });
    describe('handleLogin', () => {
        it('should login user with valid credentials', async () => {
            ...
            signSession.mockResolvedValue('mock_session_token');
            await handleLogin(mockContext);
            expect(mockContext.env.KV_USERS.put).toHaveBeenCalledWith('user:user_123', expect.stringContaining('"lastLoginAt"'));
            expect(mockContext.json).toHaveBeenCalledWith({
                ok: true,
                user: expect.objectContaining({
        ...
        it('should return error for missing credentials', async () => {
            ...
            await handleLogin(mockContext);
            expect(mockContext.json).toHaveBeenCalledWith({ error: 'Missing email or password' }, 400);
        });
    ...
    describe('handleMe', () => {
        ...
        it('should return unauthorized for unauthenticated user', async () => {
            ...
            await handleMe(mockContext);
            expect(mockContext.json).toHaveBeenCalledWith({ user: null });
        });
        it('should return not found for non-existent user', async () => {
            const mockContext = createMockContext();
            getUidFromSession.mockResolvedValue('user_999');
    ...
    describe('handleChangePassword', () => {
        ...
        it('should return error for invalid current password', async () => {
            ...
            verifyPasswordHash.mockResolvedValue(false);
            await handleChangePassword(mockContext);
            expect(mockContext.json).toHaveBeenCalledWith({ error: 'invalid_password' }, 401);
        });
        it('should return unauthorized for unauthenticated user', async () => {
            const mockContext = createMockContext();
            getUidFromSession.mockResolvedValue(null);
