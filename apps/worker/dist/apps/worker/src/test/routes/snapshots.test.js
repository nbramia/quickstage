...
import { handleCreateSnapshot, handleFinalizeSnapshot, handleListSnapshots, handleExpireSnapshot, handleExtendSnapshot, handleRotateSnapshotPassword } from '../../routes/snapshots';
import { createMockContext, mockSnapshots } from '../mocks/context';
// Mock dependencies
vi.mock('../../auth', () => ({
...
describe('Snapshots Routes', () => {
    ...
    describe('handleCreateSnapshot', () => {
        it('should create a new snapshot successfully', async () => {
            ...
            expect(mockContext.env.KV_SNAPS.put).toHaveBeenCalledWith('snap:mock_snap_id', expect.stringContaining('"id":"mock_snap_id"'));
    ...
    describe('handleFinalizeSnapshot', () => {
        ...
        it('should return not found for non-existent snapshot', async () => {
            ...
            await handleFinalizeSnapshot(mockContext);
            expect(mockContext.json).toHaveBeenCalledWith({ error: 'bad_request', details: expect.any(Object) }, 400);
        });
    ...
    describe('handleExpireSnapshot', () => {
        it('should expire snapshot successfully', async () => {
            ...
            expect(mockContext.env.KV_SNAPS.put).toHaveBeenCalledWith('snap:snap_123', expect.stringContaining('"status":"expired"'));
            expect(mockContext.json).toHaveBeenCalledWith({
                ok: true
            });
        });
