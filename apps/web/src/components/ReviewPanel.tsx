...
export function ReviewPanel({ snapshotId, isOwner, currentUserId, onReviewUpdate }: ReviewPanelProps) {
  const [reviews, setReviews] = useState<Review[]>([]);
  const [loading, setLoading] = useState(false);
  const [showRequestModal, setShowRequestModal] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Check if current user is a reviewer for any of the reviews
  const isReviewer = reviews.some(review => 
    review.reviewers.some(participant => participant.userId === currentUserId)
  );

  // Only show the panel if user is owner or reviewer
  const shouldShowPanel = isOwner || isReviewer;
  ...
  // Don't show panel if user is not owner or reviewer
  if (!shouldShowPanel && !loading) {
    return null;
  }
