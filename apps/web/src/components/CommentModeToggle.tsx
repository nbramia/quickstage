...
export default function CommentModeToggle({
  ...
  return (
    <div className={`fixed top-4 right-4 z-40 ${className}`}>
      {/* Mobile backdrop blur when comment mode is active */}
      {isCommentMode && (
        <div className="sm:hidden fixed inset-0 bg-black/10 backdrop-blur-[1px] pointer-events-none -z-10" />
      )}
      ...
      <div className="flex items-center space-x-2">
        ...
        {commentCount > 0 && (
          <button
            ...
            aria-label={`${showPins ? 'Hide' : 'Show'} comment pins on page`}
          >
            <svg 
              className="w-4 h-4" 
              fill="none" 
