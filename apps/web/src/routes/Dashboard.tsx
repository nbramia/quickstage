...
export default function Dashboard() {
  ...
  const handleShowSessionToken = async () => {
    try {
      // Get the session token from the current session
      const response = await fetch('/api/me', { credentials: 'include' });
      if (response.ok) {
        const data = await response.json();
        // Extract the session token from the response headers or cookies
        const cookies = document.cookie;
        const sessionMatch = cookies.match(/ps_sess=([^;]+)/);
        if (sessionMatch && sessionMatch[1]) {
          setSessionToken(sessionMatch[1]);
          setShowSessionToken(true);
        } else {
          setSessionToken('Session token not found');
          setShowSessionToken(true);
        }
      }
    } catch (error) {
      console.error('Failed to get session token:', error);
      setSessionToken('Failed to get session token');
      setShowSessionToken(true);
    }
  };
      ...
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div className="px-4 sm:px-0 mb-8">
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <button
              onClick={() => handleShowSessionToken()}
              ...
              <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
                <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
                  <div className="mt-3">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="text-lg font-medium text-gray-900">
                        Session Token for VS Code/Cursor Extension
                      </h3>
                      <button
                    ...
                    <div className="space-y-6">
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <h4 className="font-medium text-blue-900 mb-2">🔑 How to Use This Token</h4>
                        <p className="text-sm text-blue-800">
                          Copy this session token and paste it into your VS Code/Cursor extension when prompted. 
                          This will authenticate your extension with QuickStage.
                        </p>
                      </div>

                      <div>
                        <h4 className="font-medium text-gray-900 mb-2">Your Session Token</h4>
