...
export default function CommentOverlay({ snapshotId, isInteractive = true, onCommentModeChange, className = '' }) {
    ...
    const handleCommentModeChange = (enabled) => {
    ...
    const handleOverlayClick = (event) => {
        ...
        if (event.target.closest('.comment-pin, .comment-controls, .comment-modal')) {
            return;
        }
        ...
        if (!elementBelow || elementBelow === overlayRef.current)
            return;
        // Generate selector for the clicked element
        const elementSelector = generateSelector(elementBelow);
    ...
    return (_jsxs(_Fragment, { children: [isInteractive && (_jsx(CommentModeToggle, { isCommentMode: isCommentMode, onCommentModeChange: handleCommentModeChange, showPins: showPins, onShowPinsChange: setShowPins, commentCount: totalCommentCount, className: "comment-controls" })), _jsxs("div", { ref: overlayRef, className: `absolute inset-0 pointer-events-none z-30 ${className} ${isCommentMode ? 'cursor-crosshair' : ''}`, onClick: handleOverlayClick, style: { pointerEvents: isCommentMode ? 'auto' : 'none' }, children: [visiblePins.map((pin) => (_jsx(CommentPin, { id: pin.id, x: pin.x, y: pin.y, comments: pin.comments, isResolved: pin.isResolved, onClick: handlePinClick, className: "comment-pin" }, pin.id))), isCommentMode && (_jsx("div", { className: "absolute inset-0 bg-blue-500/5 pointer-events-none" }))] }), showCommentModal && (_jsx(CommentModal, { isOpen: showCommentModal, onClose: handleCloseModal, onSubmit: handleCommentSubmit, snapshotId: snapshotId, existingComments: activePin ?
                    commentPins.find(p => p.id === activePin)?.comments || [] :
                    [], position: pendingComment ? {
                    x: pendingComment.x,
                    y: pendingComment.y,
                    elementSelector: pendingComment.elementSelector
                } : undefined, className: "comment-modal" }))] }));
}
